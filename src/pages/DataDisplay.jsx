...
const DataDisplay = (
    ...
    const columns = [
        ...
        {key: COLUMNS.NAME, label: LABELS[COLUMNS.NAME], flex: 4, align: 'left'},
        {key: COLUMNS.ROLE, label: LABELS[COLUMNS.ROLE], flex: 2, align: 'center'},
        {key: COLUMNS.TOTAL_SCORE, label: LABELS[COLUMNS.TOTAL_SCORE], flex: 3, align: 'right'}
    ];

    const getSortIcon = (columnKey) => {
        if (config.sort.key !== columnKey) return null;

        return config.sort.direction === SORT.ASC
            ? <SortIcon active>&uarr;</SortIcon>
            : <SortIcon active>&darr;</SortIcon>;
    };
